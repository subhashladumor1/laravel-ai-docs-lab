...
.topbar-badge{background:rgba(124,77,255,.15);border:1px solid var(--border);
  border-radius:20px;padding:3px 12px;font-size:11px;color:var(--accent);letter-spacing:.5px;font-weight:600}
...
.hero h1{font-size:clamp(28px,4vw,48px);font-weight:800;line-height:1.2;
  background:linear-gradient(135deg,#fff 0%,var(--accent) 50%,var(--accent2) 100%);
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:12px}
.hero p{color:var(--muted);font-size:16px;max-width:600px;margin:0 auto 28px}
...
.gc-label{font-size:13px;font-weight:600;color:var(--muted);letter-spacing:.5px}
...
select{padding:8px 32px 8px 12px;appearance:none;cursor:pointer;min-width:160px}
input[type=text]{padding:8px 14px;width:100%}
textarea{padding:10px 14px;width:100%;resize:vertical;min-height:72px}
select:focus,input:focus,textarea:focus{
  border-color:var(--accent);box-shadow:0 0 0 3px rgba(124,77,255,.15)}
select:hover,input:hover,textarea:hover{border-color:rgba(124,77,255,.4)}
...
select::-webkit-scrollbar-thumb { background: var(--accent); border-radius: 4px; }

/* ── TABS ── */
.tabs-nav{display:flex;gap:6px;margin-bottom:28px;flex-wrap:wrap}
.tab-btn{
  padding:10px 22px;border-radius:30px;border:1px solid var(--border);
  background:transparent;color:var(--muted);font-size:13px;font-weight:600;
  cursor:pointer;transition:all .2s;letter-spacing:.3px;display:flex;align-items:center;gap:8px
}
.tab-btn:hover{border-color:var(--accent);color:var(--text)}
.tab-btn.active{
  background:linear-gradient(135deg,var(--accent),rgba(0,229,255,.8));
  border-color:transparent;color:#fff;box-shadow:0 4px 20px rgba(124,77,255,.4)
}
...
.section-icon.pdf-icon{background:rgba(255,64,129,.15);border:1px solid rgba(255,64,129,.25)}
.section-icon.img-icon{background:rgba(0,229,255,.15);border:1px solid rgba(0,229,255,.25)}
.section-icon.aud-icon{background:rgba(0,200,83,.15);border:1px solid rgba(0,200,83,.25)}
.section-icon.doc-icon{background:rgba(255,171,64,.15);border:1px solid rgba(255,171,64,.25)}
...
.test-card:hover{border-color:rgba(124,77,255,.35);box-shadow:0 4px 30px rgba(124,77,255,.12)}
.card-head{
...
.badge-get{background:rgba(0,229,255,.12);color:var(--accent2);border:1px solid rgba(0,229,255,.2)}
.badge-post{background:rgba(124,77,255,.12);color:var(--accent);border:1px solid rgba(124,77,255,.2)}
.card-head-title{font-size:14px;font-weight:600;flex:1}
.card-head-desc{font-size:11px;color:var(--muted);margin-top:2px}
.card-chevron{color:var(--muted);transition:transform .25s;margin-left:auto;flex-shrink:0;font-size:12px}
.test-card.expanded .card-chevron{transform:rotate(180deg)}
.card-body{display:none;padding:20px}
.test-card.expanded .card-body{display:block}
.card-api-path{
  font-family:'JetBrains Mono',monospace;font-size:11px;
  color:var(--accent2);background:rgba(0,229,255,.06);
...
.drop-zone:hover,.drop-zone.drag-over{
  border-color:var(--accent);background:rgba(124,77,255,.06)}
...
.file-preview{
  ...
  background:rgba(124,77,255,.08);border-radius:8px;padding:8px 12px;
}
.file-preview-icon{font-size:18px}
.file-preview-name{font-size:12px;font-weight:600;flex:1;word-break:break-all}
.file-preview-size{font-size:11px;color:var(--muted)}
.file-preview-clear{
  background:none;border:none;color:var(--muted);cursor:pointer;
  font-size:16px;padding:2px 6px;border-radius:4px;
}
.file-preview-clear:hover{color:var(--error);background:rgba(255,82,82,.1)}

/* ── BUTTONS ── */
.btn{
  display:inline-flex;align-items:center;gap:8px;
  padding:10px 22px;border-radius:30px;font-weight:600;font-size:13px;
  cursor:pointer;border:none;transition:all .2s;width:100%;justify-content:center;
}
.btn-primary{
  background:linear-gradient(135deg,var(--accent),rgba(0,229,255,.9));
  color:#fff;box-shadow:0 4px 16px rgba(124,77,255,.3)
}
.btn-primary:hover{transform:translateY(-1px);box-shadow:0 6px 24px rgba(124,77,255,.4)}
.btn-primary:active{transform:translateY(0)}
.btn-primary:disabled{opacity:.5;cursor:not-allowed;transform:none}
.btn-sm{padding:6px 14px;font-size:12px;width:auto;border-radius:20px}
...
.result-status.ok{background:rgba(0,200,83,.1);color:var(--success);border:1px solid rgba(0,200,83,.2)}
.result-status.err{background:rgba(255,82,82,.1);color:var(--error);border:1px solid rgba(255,82,82,.2)}
.result-duration{font-size:11px;color:var(--muted);margin-left:auto}
.copy-btn{
  font-size:11px;padding:4px 10px;background:rgba(255,255,255,.06);
  border:1px solid var(--border);border-radius:16px;color:var(--muted);
  cursor:pointer;transition:all .15s;font-weight:600
}
.copy-btn:hover{border-color:var(--accent);color:var(--accent)}
.result-content{
  background:rgba(0,0,0,.3);border:1px solid var(--border);border-radius:var(--radius-sm);
  font-family:'JetBrains Mono',monospace;font-size:12px;line-height:1.6;
  padding:16px;max-height:340px;overflow-y:auto;white-space:pre-wrap;word-break:break-word;
  color:#c8d8e8;
}
.result-content::-webkit-scrollbar{width:4px}
.result-content::-webkit-scrollbar-track{background:transparent}
.result-content::-webkit-scrollbar-thumb{background:var(--accent);border-radius:4px}
...
.keys-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;margin-bottom:32px}
.key-card{
  background:var(--card);border:1px solid var(--border);border-radius:var(--radius-sm);
  padding:14px 18px;display:flex;align-items:center;gap:12px
}
.key-indicator{width:10px;height:10px;border-radius:50%;flex-shrink:0}
.key-indicator.has-key{background:var(--success);box-shadow:0 0 8px var(--success)}
.key-indicator.no-key{background:var(--error);box-shadow:0 0 8px var(--error)}
.key-info{}
.key-name{font-size:13px;font-weight:600}
.key-status{font-size:11px;color:var(--muted);margin-top:2px}

/* ── QUICK TIPS ── */
.tips-box{
  background:rgba(124,77,255,.06);border:1px solid rgba(124,77,255,.18);
  border-radius:var(--radius-sm);padding:14px 18px;margin-bottom:16px;font-size:12px;
}
.tips-box code{font-family:'JetBrains Mono',monospace;
  background:rgba(124,77,255,.15);padding:1px 6px;border-radius:4px;font-size:11px}

/* ── RESPONSIVE ── */
@media(max-width:700px){
  .topbar{padding:12px 16px}.hero{padding:36px 16px 24px}
  .cards-grid{grid-template-columns:1fr}.global-config{flex-direction:column;align-items:flex-start}
  .tabs-nav{gap:4px}.tab-btn{padding:8px 14px;font-size:12px}
}
</style>
</head>
<body>
...
<div class="container">
  ...
  <div class="hero">
    ...
    <p>Test every method of the <strong>laravel-ai-docs</strong> package in real-time. Upload files, choose a model, and see live AI results.</p>
    <div class="hero-stats">
      <div class="stat"><div class="stat-num">4</div><div class="stat-label">Document Types</div></div>
      <div class="stat"><div class="stat-num">23</div><div class="stat-label">API Methods</div></div>
      <div class="stat"><div class="stat-num">3</div><div class="stat-label">AI Providers</div></div>
      <div class="stat"><div class="stat-num">23</div><div class="stat-label">AI Models</div></div>
    </div>
  </div>
  ...
  <div class="global-config">
    ...
    <span style="font-size:12px;color:var(--muted)">These apply to all tests unless individually overridden</span>
  </div>

  <!-- TABS -->
  <div class="tabs-nav">
    <button class="tab-btn active" data-tab="pdf">📄 PDF Builder <span style="background:rgba(255,64,129,.15);color:#ff4081;border-radius:20px;padding:1px 8px;font-size:10px">8</span></button>
    <button class="tab-btn" data-tab="image">🖼️ Image Builder <span style="background:rgba(0,229,255,.12);color:var(--accent2);border-radius:20px;padding:1px 8px;font-size:10px">6</span></button>
    <button class="tab-btn" data-tab="audio">🎙️ Audio Builder <span style="background:rgba(0,200,83,.12);color:var(--success);border-radius:20px;padding:1px 8px;font-size:10px">3</span></button>
    <button class="tab-btn" data-tab="document">📝 Document Builder <span style="background:rgba(255,171,64,.12);color:var(--warning);border-radius:20px;padding:1px 8px;font-size:10px">7</span></button>
  </div>
  ...
  <div class="tab-panel active" id="tab-pdf">
    ...
    <div class="tips-box">
      💡 <strong>Tip:</strong> Use <code>AIDocs::pdf($file)->text()</code> for raw text, <code>->summarize()->text()</code> for AI summary, <code>->ask('question')</code> for RAG Q&A, <code>->toJson()</code> for structured extraction, <code>->pages()</code> for page count, <code>->tables()->result()</code> for table extraction, <code>->toMarkdown()</code> for markdown output, and <code>->enhance()->tables()->summarize()->result()</code> for the full DocumentResultDTO pipeline.
    </div>
  ...
  <div class="tab-panel" id="tab-image">
    <div class="section-header">
      ...
      <div class="section-title">
        ...
        <p>AIDocs::image($path) → OCR and AI vision. Supports JPG, PNG, WEBP, GIF, BMP, TIFF up to 10 MB.</p>
      </div>
    </div>
    <div class="tips-box">
      💡 <strong>Tip:</strong> ImageBuilder uses <strong>lazy extraction</strong> — text is only extracted when you call a terminal method. Use <code>->text(?$prompt)</code> for OCR, <code>->summarize()</code> for description, <code>->ask('question')</code> for Q&A, <code>->tables()</code> for table extraction, <code>->toJson()</code> for structured data, and <code>->result()</code> for the full DocumentResultDTO.
    </div>
    <div class="cards-grid">
  ...
  <div class="tab-panel" id="tab-audio">
    <div class="section-header">
      ...
      <div class="section-title">
        ...
        <p>AIDocs::audio($path) → OpenAI Whisper transcription. Supports MP3, MP4, M4A, WAV, WEBM, OGG up to 25 MB.</p>
      </div>
    </div>
    <div class="tips-box">
      ⚠️ <strong>Note:</strong> Audio transcription requires <strong>OpenAI API key</strong> (Whisper). Claude and Gemini do not support direct audio transcription. Use <code>->transcribe()</code> for raw transcript, <code>->summarize()</code> to get a summary, and <code>->result()</code> for the full DocumentResultDTO with both transcript and metadata.
    </div>
    <div class="cards-grid">
  ...
  <div class="tab-panel" id="tab-document">
    ...
    <div class="tips-box">
      💡 <strong>Tip:</strong> <code>AIDocs::document($path)</code> is a universal entry point — it auto-detects the file type. Use it for DOCX/DOC instead of <code>AIDocs::pdf()</code>. Methods: <code>->text()</code>, <code>->summarize()->text()</code>, <code>->ask()</code>, <code>->toJson()</code>, <code>->toMarkdown()</code>, <code>->tables()->result()</code>, and the full pipeline <code>->enhance()->tables()->summarize()->result()</code>.
    </div>
    <div class="cards-grid">
